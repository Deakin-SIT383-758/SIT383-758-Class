...
public class HandDetection : MonoBehaviour
{
    ...
    public float MiddleThumb_EndIndexDist = 0.5f;
    public float Fist_Theshold = 0.4f;
    ...
    async Awaitable Detect(Texture texture)
    {
        ...
        Vector3 ThumbMiddle = jointPositions[3];
        Vector3 thumpTip = jointPositions[4];
        Vector3 IndexEnd = jointPositions[6];
        Vector3 indexTip = jointPositions[8];

        //Fist tracker
        Vector3 MiddlefingerMcp = jointPositions[9];
        Vector3 MiddlefingerTip = jointPositions[12];
        Vector3 RingfingerMcp = jointPositions[13];
        Vector3 RingfingerTip = jointPositions[16];
        Vector3 PinkyfingerMcp = jointPositions[17];
        Vector3 PinkyfingerTip = jointPositions[20];

        Vector3 indexDirection = (indexTip - wrist).normalized;
        float angle = Mathf.Atan2(indexDirection.y, indexDirection.x) * Mathf.Rad2Deg;
        Quaternion IndexRotation = Quaternion.Euler(0f, 0f, angle);

        float normalisedFTDistance = (indexTip - thumpTip).magnitude / (thumpTip - wrist).magnitude;
        float normalisedMiddle = (IndexEnd - ThumbMiddle).magnitude / (ThumbMiddle - wrist).magnitude;

        float normalisedMFDistance = (MiddlefingerTip - MiddlefingerMcp).magnitude / (MiddlefingerTip - wrist).magnitude;
        float normalisedRFDistance = (RingfingerTip - RingfingerMcp).magnitude / (RingfingerTip - wrist).magnitude;
        float normalisedPFDistance = (PinkyfingerTip - PinkyfingerMcp).magnitude / (PinkyfingerTip - wrist).magnitude;
        //Debug.Log("Distance:" + normalisedPFDistance);

        if (normalisedFTDistance > pinchThreshold && normalisedMiddle > MiddleThumb_EndIndexDist)
        {
            if (normalisedMFDistance < Fist_Theshold && normalisedRFDistance < Fist_Theshold && normalisedPFDistance < Fist_Theshold)
            {
                Instantiate(Bullets, IndexEnd, IndexRotation);
            }
        }
    }
