...
public class HandDetection : MonoBehaviour
{
    ...
    async Awaitable Detect(Texture texture)   //Hand gesture components within this function
    {
        ...
        Vector3[] joinPositions = new Vector3[k_NumKeypoints];
        ...
        for (var i = 0; i < k_NumKeypoints; i++)
        {
            ...
            joinPositions[i] = position_WorldSpace;
        }


        //Pinch Detection
        Vector3 thumbTip = joinPositions[4];
        Vector3 indexTip = joinPositions[8];
        Vector3 wrist = joinPositions[0];

        float noramalizedFTDistance = (indexTip - thumbTip).magnitude / (wrist - thumbTip).magnitude;

        //Debug.Log("Tip Distance: " + noramalizedFTDistance);

        //if (noramalizedFTDistance < pinchThreshold)
        //Debug.Log("Pinch detected!");

        // --- Improved Gesture Recognition: Rock, Paper, Scissors ---

        // Helper function to compute the angle between three points (in degrees)
        float FingerAngle(Vector3 a, Vector3 b, Vector3 c)
        {
            Vector3 ab = (a - b).normalized;
            Vector3 cb = (c - b).normalized;
            return Mathf.Acos(Mathf.Clamp(Vector3.Dot(ab, cb), -1f, 1f)) * Mathf.Rad2Deg;
        }

        // Indices for finger joints
        int[][] fingerJoints = new int[][]
        {
            new int[] {2, 3, 4},    // Thumb: MCP, IP, TIP
            new int[] {5, 6, 8},    // Index: MCP, PIP, TIP
            new int[] {9, 10, 12},  // Middle: MCP, PIP, TIP
            new int[] {13, 14, 16}, // Ring: MCP, PIP, TIP
            new int[] {17, 18, 20}  // Pinky: MCP, PIP, TIP
        };

        bool[] isFingerExtended = new bool[5];
        for (int i = 0; i < 5; i++)
        {
            float angle = FingerAngle(
                joinPositions[fingerJoints[i][0]],
                joinPositions[fingerJoints[i][1]],
                joinPositions[fingerJoints[i][2]]
            );
            // If the angle at the PIP (or IP for thumb) is large, the finger is extended
            isFingerExtended[i] = angle > 160f; // Threshold can be tuned
        }

        // Check finger separation for paper and scissors
        float indexMiddleDist = (joinPositions[8] - joinPositions[12]).magnitude;
        float handSize = (joinPositions[0] - joinPositions[9]).magnitude; // Wrist to middle MCP
        bool indexMiddleApart = indexMiddleDist > handSize * 0.4f; // Tune as needed

        if (!isFingerExtended[1] && !isFingerExtended[2] && !isFingerExtended[3] && !isFingerExtended[4])
        {
            gestureText.text = "Rock";
            gestureText.color = Color.gray;
        }
        else if (isFingerExtended[1] && isFingerExtended[2] && isFingerExtended[3] && isFingerExtended[4]
                 && indexMiddleApart
                 && isFingerExtended[0])
        {
                 ...
                 && indexMiddleApart)
        {
            ...
            gestureText.color = Color.blue;
        }
        else
        {
            gestureText.text = ""; // Or "No gesture"
        }
