...
public class ObjectScan : MonoBehaviour
{
    ...
    void CreateMesh(int width, int height)
    {
        ...
        tris = new int[6 * ((width - 1) * (height - 1))];
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                if (x != (width - 1) && y != (height - 1))
                {
                    ...
                    tris[trisIndex++] = topLeft;
                    tris[trisIndex++] = topRight;
                    tris[trisIndex++] = bottomLeft;
                    tris[trisIndex++] = bottomLeft;
                    tris[trisIndex++] = topRight;
                    tris[trisIndex++] = bottomRight;
                }
            }
        }
    ...
    IEnumerator updateMesh()
    {
        while (true)
        {
            ...
            if (timerStarted && (timeDelay < 0.0f))
            {
                ...
                // For continuous capture. Recommend timeBeforeCaptures of at least 1 second
                // timeDelay = timeBeforeCapture;
                // timerStarted = true;
    ...
    private void RefreshData(Texture depthData)
    {
        ...
        int xSteps = Mathf.Max(readTex.width, meshX); // deal with cases where mesh > textures or vice versa
    ...
    void exportObj(string filenameBase)
    {
        ...
        using (System.IO.StreamWriter file = new System.IO.StreamWriter(filenameBase + ".obj"))
        {
            ...
            {
                file.WriteLine("f " + (1 + tris[i + 0]) + "/" + (1 + tris[i + 0]) + "/" + (1 + tris[i + 0]) + " " +
                                    (1 + tris[i + 1]) + "/" + (1 + tris[i + 1]) + "/" + (1 + tris[i + 1]) + " " +
                                    (1 + tris[i + 2]) + "/" + (1 + tris[i + 2]) + "/" + (1 + tris[i + 2]));
            }
        }
