...
public class HandDetection : MonoBehaviour
{
    ...
    public float spreadThreshold = 7.5f;
    public float fistThreshold = 200.0f;
    ...
    async Awaitable Detect(Texture texture)
    {
        ...
        Vector3[] joinPosi = new Vector3[k_NumKeypoints];
        ...
        for (var i = 0; i < k_NumKeypoints; i++)
        {
            ...
            joinPosi[i] = position_WorldSpace;
        }

        Vector3 wrist = joinPosi[0];
        Vector3 thumbTip = joinPosi[4];
        Vector3 indexTip = joinPosi[8];
        Vector3 middleTip = joinPosi[12];
        Vector3 ringTip = joinPosi[16];
        Vector3 pinkyTip = joinPosi[20];

        float normalizedPinchDistance = (indexTip-thumbTip).magnitude / (thumbTip-wrist).magnitude;
        float normalizedHandSpread = (thumbTip-wrist).magnitude / (indexTip-wrist).magnitude / (middleTip-wrist).magnitude / (ringTip-wrist).magnitude / (pinkyTip-wrist).magnitude;

        if (normalizedHandSpread > fistThreshold && normalizedHandSpread < 450f)
        {
            fistDetected = true;
            Debug.Log("Rock = " + fistDetected);
        }
        else {fistDetected = false;}
        if (normalizedPinchDistance < pinchThreshold && fistDetected == false)
        {
            pinchDetected = true;
            Debug.Log("Pinch = " + pinchDetected);
        }
        else {pinchDetected = false;}
        if (normalizedHandSpread > spreadThreshold && normalizedHandSpread < 9f && pinchDetected == false)
        {
            spreadDetected = true;
            Debug.Log("Paper = " + spreadDetected);
        }
        else {spreadDetected = false;}
    }
